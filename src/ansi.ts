...
export function parseAnsiStyledLogo(logo: string): AnsiStyledRun[] {
  const runs: AnsiStyledRun[] = []
  let style: AnsiStyle = {}
  let text = ""
...
function applySgrParameters(current: AnsiStyle, parameters: number[]): AnsiStyle | undefined {
  let style = { ...current }
  ...
  for (let position = 0; position < parameters.length; position += 1) {
    ...
    if (parameter === 0) {
      ...
      if (!color) return
      if (parameter === 38) style.foreground = color.value
...
type ParsedExtendedColor = {
  value: AnsiColor
  lastPosition: number
}

function readExtendedColor(parameters: number[], position: number): ParsedExtendedColor | undefined {
  const mode = parameters[position]
  if (mode === 5) {
    const value = parameters[position + 1]
    if (!isByte(value)) return
  ...
  if (mode === 2) {
    ...
    if (!isByte(red) || !isByte(green) || !isByte(blue)) return
